// FIXED streamManager.js - Multi-stream timer fix
...
class StreamManager extends EventEmitter {
    constructor(database) {
        ...
        // FIX: Tách riêng Map để quản lý timers
    ...
    async startStream(config) {
        ...
        try {
            ...
            // FIX: Setup auto-stop timer - Sử dụng Map riêng
            if (config.stopAfterMinutes) {
                this.setupDurationTimer(streamId, config.stopAfterMinutes);
            } else if (config.stopAtTime) {
                this.setupSpecificTimeTimer(streamId, config.stopAtTime);
            }
            ...
            // FIX: Clean up timers nếu có lỗi
            this.clearStreamTimers(streamId);
            throw error;
        }
    }

    // FIX: Hàm riêng để setup timer theo duration
    ...
    // FIX: Hàm riêng để setup timer theo giờ cụ thể
    setupSpecificTimeTimer(streamId, timeString) {
        const streamInfo = this.activeStreams.get(streamId);
        if (!streamInfo) return;
        ...
        // Set new timer
        const timer = setTimeout(() => {
            console.log(`⏰ Auto-stopping stream ${streamId} at scheduled time ${timeString}`);
            this.stopStream(streamId);
        }, delay);
        ...
        console.log(`✅ [${streamId}] Timer set: Stream will stop at ${timeString}`);
    }

    // FIX: Hàm riêng để clear tất cả timers của một stream
    ...
    updateTimerCountdown(streamId) {
        const streamInfo = this.activeStreams.get(streamId);
        if (!streamInfo || !streamInfo.timerInfo) {
            // Stream không còn hoặc không có timer - clear interval
            this.clearStreamTimers(streamId);
            return;
        }
        ...
        setTimeout(async () => {
            if (this.activeStreams.has(streamId)) {
                try {
                    ...
                    // FIX: KHÔNG clear timers khi restart - giữ nguyên timer
    ...
    async stopStream(streamId) {
        ...
        try {
            ...
            // FIX: Clear all timers using dedicated function
            this.clearStreamTimers(streamId);
    ...
    async stopAllStreams() {
        ...
        // FIX: Double-check to clear all timers
